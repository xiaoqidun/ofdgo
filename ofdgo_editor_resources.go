...
// pruneSourceResources 在保存副本中清理无引用字体和图片，保留其他资源及原始编辑状态。
...
func (e *Editor) pruneSourceResources(parts map[string][]byte) (map[string]bool, error) {
	if len(parts) == 0 {
		return nil, nil
	}
				...
				if file := node.child(field); file != nil {
					...
					if location := editorResourceLocation(name, root.attr("BaseLoc"), value); location != "" {
						if keep {
							...
							usedFiles[strings.ToLower(cleanPackagePath(resolveResourcePath(name, root.attr("BaseLoc"), value)))] = true
						} else {
