...
func (e *Editor) UpdateCompositeText(page int, path ObjectPath, index int, value string, style TextStyle) error {
	return e.changeCompositeText(page, path, []int{index}, &value, style)
}
...
func (e *Editor) StyleCompositeText(page int, path ObjectPath, indexes []int, style TextStyle) error {
	return e.changeCompositeText(page, path, indexes, nil, style)
}

// changeCompositeText 生成独立文字副本，全部校验通过后合并原文
// 入参: page 页面索引, path 父复合路径, indexes 成员序号, value 新内容，nil保留定位, style 样式增量
// 返回: error 错误信息
func (e *Editor) changeCompositeText(page int, path ObjectPath, indexes []int, value *string, style TextStyle) error {
	return e.editCompositeObjects(page, path, indexes, func(_ *Renderer, nodes []*editorCompositeNode, members []CompositeMember) error {
		for i, node := range nodes {
			...
			if content != text.Text() || style.Size != 0 && style.Size != text.Size {
				if style.Size != 0 {
					text.Size = style.Size
				}
				if err := e.layoutCompositeText(&text, content); err != nil {
					return err
				}
...
// 入参: text 有效文字样式, value 新内容
// 返回: error 错误信息
func (e *Editor) layoutCompositeText(text *TextObject, value string) error {
	...
	if err := e.LayoutText(text, value, TextLayout{}); err != nil {
		return err
	}
